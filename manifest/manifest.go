...
type SourceConnectorSpec struct {
	...
	// EdgeCompleteness declares that this connector's artifacts are the COMPLETE
	// edge set for the assets they carry, so a relationship it stops declaring has
	// ended and the platform closes it (OGA-930). Optional: absent means the
	// connector asserts nothing and no edge it omits is ever closed.
	//
	// Declared HERE rather than on each artifact because a connector runs
	// continuously with no operator in the loop — there is no per-run action to hang
	// the assertion off. The platform turns this into a STANDING record at install,
	// keyed on the connector's verified identity, and the connector plays no part in
...
// complete edge sets, and the vocabulary that declaration covers (OGA-930).
//
// Mirrors the platform's domainkit.EdgeCompletenessSpec field-for-field, so a kit
// author gets the same result from a local manifest.Parse as from an install.
//
// The mode and the vocabulary are ONE block because neither is meaningful alone: a
// mode with no vocabulary licenses nothing, and a vocabulary with no mode is inert.
// The platform REFUSES a half-declaration rather than deploying a feed that will
// never converge.
type EdgeCompletenessSpec struct {
	// Mode is the completeness claim. The only accepted value is "per_source": for
	...
	// feed owns, generated by the kit's build from its own predicate map — the same
	// convention as an ontology-types file.
	//
	// A FILE rather than an inline list because a hand-maintained list drifts
	// silently in the UNSAFE direction: a predicate still listed but no longer
	// emitted becomes a predicate the platform will close. Accepts a flat JSON array
	// or {"predicates": [...]}.
	GovernedPredicatesFile string `yaml:"governed_predicates_file,omitempty"`
}
